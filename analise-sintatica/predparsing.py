        ...
        if s in self.terminals:
            ...
            for rhs in self.production_rules[s]:
                ...
                for i in range(1, len(rhs)):
                    ...
                    ### Do your magic!
                    if 'epsilon' not in self.first_tab[y_ant]:
                        break
                    
                    if self.first_tab[y_i] == set():
                        self.first(y_i)                   

                    for f in self.first_tab[y_i]:
                        self.first_log(s, f, rhs)
                        self.first_tab[s].add(f)
    ...
    def compute_follow(self):
        ...
        while True:
            ...
            for nt in self.non_terminals: 
                ft_ant = copy.deepcopy(self.follow_tab)
                self.follow(nt)                
                if not somethingAdded:
                    for entry in ft_ant:
                        if len(ft_ant[entry]) != len(self.follow_tab[entry]):
                            somethingAdded = True
                            break
            if not somethingAdded:
                break
            ...
            for i in range(len(rhs)):
                ...
                if B in self.non_terminals:
                    ...
                    if beta != ():
                        ...
                        ### Do your magic!
                        firstBetaContainsEpsilon = True 
                        for b in beta:
                            if 'epsilon' in self.first_tab[b]:
                                self.follow_tab[B].update(self.first_tab[b] - {'epsilon'})
                            else:    
                                self.follow_tab[B].update(self.first_tab[b])
                                firstBetaContainsEpsilon = False
                                break
                ...
                if alpha == ("epsilon",) or \
                   (alpha != ("epsilon",) and ("epsilon" in self.firstW(alpha))):
                    ...
                    for b in self.follow_tab[A]:
                        self.pred_parsing_tab[A][b].append(str(A) + ' -> ' + ''.join(alpha))
                    ...
                    for a in self.firstW(alpha):
                        self.pred_parsing_tab[A][a].append(str(A) + ' -> ' + ''.join(alpha))
